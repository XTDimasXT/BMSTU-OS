...
bakery(void *arg)
{
    ...
    printf("Thread id = '%lu' started, client's number = %d, time = %s", tid, number[targ->id], asctime(timeinfo));
	int i = targ->id;
	for (int j = 0; j < 30; j++)
	{
		while (choosing[j]);
	...
	time(&raw_time);
    timeinfo = localtime(&raw_time);
	printf("Thread id = '%lu' stopped, client's number = 0, time = %s --got symbol = %c\n", tid, asctime(timeinfo), symbol-1);
	number[i] = 0;
	return 0;
}
